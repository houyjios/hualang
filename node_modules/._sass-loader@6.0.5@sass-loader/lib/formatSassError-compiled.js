...
var SassError = { // eslint-disable-line no-unused-vars
...
function formatSassError(err, resourcePath) {
    ...
    err.message = getFileExcerptIfPossible(err) + msg.charAt(0).toUpperCase() + msg.slice(1) + os.EOL + "      in " + err.file + " (line " + err.line + ", column " + err.column + ")";
}
...
function getFileExcerptIfPossible(err) {
    try {
        ...
        return os.EOL + content.split(os.EOL)[err.line - 1] + os.EOL + new Array(Math.max(err.column - 1, 0)).join(" ") + "^" + os.EOL + "      ";
    } catch (err) {
...
//# sourceMappingURL=formatSassError-compiled.js.map