...
var importsToResolve = require("./importsToResolve");
...
function webpackImporter(resourcePath, resolve, addNormalizedDependency) {
    ...
    function startResolving(dir, importsToResolve) {
        return importsToResolve.length === 0 ? Promise.reject() : resolve(dir, importsToResolve[0]).then(function (resolvedFile) {
            ...
            return startResolving(dir, tail(importsToResolve));
        });
    }

    return function (url, prev, done) {
        startResolving(dirContextFrom(prev), importsToResolve(utils.urlToRequest(url))) // Catch all resolving errors, return the original file and pass responsibility back to other custom importers
        .catch(function () {
            return { file: url };
        }).then(done);
    };
...
//# sourceMappingURL=webpackImporter-compiled.js.map