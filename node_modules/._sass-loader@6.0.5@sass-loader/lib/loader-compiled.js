...
var formatSassError = require("./formatSassError");
var webpackImporter = require("./webpackImporter");
...
function sassLoader(content) {
    ...
    var options = normalizeOptions(this, content, webpackImporter(resourcePath, pify(this.resolve.bind(this)), addNormalizedDependency));
    ...
    asyncSassJobQueue.push(options, function (err, result) {
        if (err) {
            formatSassError(err, _this.resourcePath);
            err.file && _this.dependency(err.file);
            callback(err);
            return;
        }
